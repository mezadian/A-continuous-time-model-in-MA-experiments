...
int main(int argc, char** argv) {
    ...
    double mutotal;
    double selective_effect[MAXTYPES - 1], death_mu[MAXTYPES - 1], birth_rate[MAXTYPES - 1];
    ...
    for (int i = 0; i < nNtypes; i++) {
        ...
        death_mu[i] = death;
        birth_rate[i] = log(1.0 + selective_effect[i])/(2.0 * (1.0 - death)) + 1.0;
        if (birth_rate[i] <= 0.0) {
            birth_rate[i] = 0.0;
        }
        else {
        ...
        //printf("death_mu: %f\n");
        //printf("selective_effect, death_mu, birth_rate: %f %f %f\n", selective_effect[i],death_mu[i], birth_rate[i]);
    }
    
    //printf("birth_rate[ianc], selective_effect[ianc]: %f %f\n", birth_rate[1200], selective_effect[1200]);
        ...
        for (itransfer = 1;itransfer <= ntransfers;itransfer++) {
            ...
            //do {
                ...
                while (t < ngens) {
                    ...
                    for (int j = 0; j < nNtypes; j++) {
                        A[j] = Ntypes_total[j] * birth_rate[j];
                        //printf("A: %f\n", A[j]);
                    ...
                    //
                    // nbabies = bnldev((float)(birth_rate[bin_index]), (int)(2), &seed);
                    nbabies = bnldev((float)(1-death_mu[bin_index]), (int)(2), &seed);
                    //printf("nbabies: %d\n", nbabies);
